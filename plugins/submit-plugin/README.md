...
- **Git**
- **[GitHub CLI](https://cli.github.com/) (`gh`), authenticated** — run `gh auth login` if `gh auth status` doesn't already report you as logged in. The workflow checks both before doing anything else, so a missing or unauthenticated `gh` fails fast instead of after the security check and docs build have already run.
