...
```
/plugin install secret-resolver@Larkin-Jainschigg-Marketplace
```

After installing, run setup to confirm `uv` is present and pick a backend:

```
/secret-resolver setup
```
...
/secret-resolver install                         — register this resolver so other tools can find it
```
