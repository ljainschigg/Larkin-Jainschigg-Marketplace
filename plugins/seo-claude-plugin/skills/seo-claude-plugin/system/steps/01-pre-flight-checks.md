...
- Confirm the `researcher` plugin is installed (this workflow calls `/research`). If it is not available, tell the user to install it with `/plugin install researcher@Larkin-Jainschigg-Marketplace` and run its `/setup`.
...
NOTE: Credentials used for filling gated PDF forms during research are stored by the researcher plugin at `${CLAUDE_PLUGIN_DATA}/config/` or `~/.claude/plugins/data/researcher-Larkin-Jainschigg-Marketplace/.env`. The researcher plugin manages these — run `/setup` within the researcher plugin if credentials have not been configured.
