...
`${CLAUDE_PLUGIN_ROOT}` expands to the plugin's installation path at runtime (e.g. `~/.claude/plugins/cache/claude-plugins/<plugin-name>/1.0.0`). Use it whenever a `SKILL.md` or `.mcp.json` needs to reference a file bundled with the plugin.

Similarly, `${CLAUDE_PLUGIN_DATA}` expands to a writable per-plugin data directory (`~/.claude/plugins/data/<plugin-name>-claude-plugins`). Use it for state that should persist across sessions — task lists, caches, user preferences.
